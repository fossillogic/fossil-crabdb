...
FOSSIL_SUITE(cpp_timeshell_fixture);

FOSSIL_SETUP(cpp_timeshell_fixture) {
    // Setup the test fixture
}

FOSSIL_TEARDOWN(cpp_timeshell_fixture) {
...
FOSSIL_TEST(cpp_test_timeshell_insert_interval) {
    ...
    fossil_timeshell_error_t result = fossil::bluecrab::TimeShell::insert(file_name, &interval);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);

    size_t count = 0;
    result = fossil::bluecrab::TimeShell::count_intervals(file_name, &count);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);
...
FOSSIL_TEST(cpp_test_timeshell_find_overlap) {
    ...
    for (size_t i = 0; i < 3; ++i)
        fossil::bluecrab::TimeShell::insert(file_name, &intervals[i]);
...
FOSSIL_TEST(cpp_test_timeshell_update_interval) {
    ...
    fossil::bluecrab::TimeShell::insert(file_name, &old_interval);

    fossil_timeshell_error_t result = fossil::bluecrab::TimeShell::update(file_name, &old_interval, &new_interval);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);
    ...
    size_t found = 0;
    result = fossil::bluecrab::TimeShell::find(file_name, &query, results, 1, &found);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);
...
FOSSIL_TEST(cpp_test_timeshell_remove_interval) {
    ...
    fossil::bluecrab::TimeShell::insert(file_name, &interval);

    fossil_timeshell_error_t result = fossil::bluecrab::TimeShell::remove(file_name, &interval);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);
...
FOSSIL_TEST(cpp_test_timeshell_backup_restore) {
    ...
    fossil_timeshell_error_t result = fossil::bluecrab::TimeShell::backup_database(file_name, backup_file);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);

    fossil::bluecrab::TimeShell::delete_database(file_name);
    result = fossil::bluecrab::TimeShell::restore_database(backup_file, file_name);
    ASSUME_ITS_TRUE(result == FOSSIL_TIMESHELL_ERROR_SUCCESS);
    ...
    fossil::bluecrab::TimeShell::delete_database(file_name);
    fossil::bluecrab::TimeShell::delete_database(backup_file);
}
...
FOSSIL_TEST(cpp_test_timeshell_validate_interval) {
    ...
    invalid.end_time = 2;
    ASSUME_ITS_TRUE(fossil::bluecrab::TimeShell::validate_interval(&valid));
    ASSUME_ITS_FALSE(fossil::bluecrab::TimeShell::validate_interval(&invalid));
}

// * * * * * * * * * * * * * * * * * * * * * * * *
// * Fossil Logic Test Pool
// * * * * * * * * * * * * * * * * * * * * * * * *
FOSSIL_TEST_GROUP(cpp_timeshell_database_tests) {
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_insert_interval);
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_find_overlap);
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_update_interval);
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_remove_interval);
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_backup_restore);
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_validate_extension);
    FOSSIL_TEST_ADD(cpp_timeshell_fixture, cpp_test_timeshell_validate_interval);

    FOSSIL_TEST_REGISTER(cpp_timeshell_fixture);
} // end of tests
