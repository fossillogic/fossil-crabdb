...
FOSSIL_TEST_SUITE(c_query_fixture);
fossil_crabdb_t *c_query_mock;

FOSSIL_SETUP(c_query_fixture) {
    c_query_mock = fossil_crabdb_create();
}

FOSSIL_TEARDOWN(c_query_fixture) {
    fossil_crabdb_destroy(c_query_mock);
}
...
FOSSIL_TEST_CASE(c_test_crabql_query_select) {
    const char *query = "SELECT * FROM users;";
    ASSUME_ITS_FALSE(fossil_crabql_query(c_query_mock, query));
}

// Test successful INSERT
FOSSIL_TEST_CASE(c_test_crabql_query_insert) {
    const char *query = "INSERT INTO users VALUES ('Alice', 30);";
    ASSUME_ITS_TRUE(fossil_crabql_query(c_query_mock, query));
}

// Test successful UPDATE
FOSSIL_TEST_CASE(c_test_crabql_query_update) {
    const char *query = "UPDATE users SET age = 31 WHERE name = 'Alice';";
    ASSUME_ITS_FALSE(fossil_crabql_query(c_query_mock, query));
}

// Test successful DELETE
FOSSIL_TEST_CASE(c_test_crabql_query_delete) {
    const char *query = "DELETE FROM users WHERE name = 'Alice';";
    ASSUME_ITS_FALSE(fossil_crabql_query(c_query_mock, query));
}

// Test invalid query
FOSSIL_TEST_CASE(c_test_crabql_query_invalid) {
    const char *query = "SELECT FROM users;";
    ASSUME_ITS_FALSE(fossil_crabql_query(c_query_mock, query));
}

// Test loading queries from a .crab file
FOSSIL_TEST_CASE(c_test_crabql_load_queries_from_file) {
    ...
    ASSUME_ITS_TRUE(fossil_crabql_load_queries_from_file(c_query_mock, filename));
...
FOSSIL_TEST_CASE(c_test_crabql_load_invalid_queries_from_file) {
    const char *filename = "invalid_queries.crab";
    // Attempt to load from a nonexistent file
    crabql_status_t status = fossil_crabql_load_queries_from_file(c_query_mock, filename);
    ASSUME_ITS_TRUE(status == CRABQL_FILE_NOT_FOUND);
}

// Test loading a .crab file with invalid queries
FOSSIL_TEST_CASE(c_test_crabql_load_invalid_queries) {
...
FOSSIL_TEST_CASE(c_test_crabql_query_delete_with_operators) {
...
FOSSIL_TEST_GROUP(c_crab_query_tests) {
    ...
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_query_delete);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_query_invalid);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_load_queries_from_file);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_load_invalid_queries_from_file);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_load_invalid_queries);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_query_select_with_operators);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_query_update_with_operators);
    FOSSIL_TEST_ADD(c_query_fixture, c_test_crabql_query_delete_with_operators);

    FOSSIL_TEST_REGISTER(c_query_fixture);
} // end of tests
